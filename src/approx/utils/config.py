...
class Config(OrderedDict):
    ...
    def _load_dict_from_file(filename):
        ...
        if BASE_KEY in cfg:
            ...
            for bfn in base_filenames:
                Config.merge_dict_b2a(
                    cfg_base,
                    Config._load_dict_from_file(os.path.join(cfg_dir, bfn)))
            cfg.pop(BASE_KEY)
    ...
    def merge_dict_b2a(a, b):
        ...
        for k, v in b.items():
            if ((not (k in a))
                    or (isinstance(v, dict) and v.get(COVER_KEY, False))
                    or (not isinstance(v, dict))
                    or (not isinstance(a[k], dict))):
    ...
    def dfs(self, cfg_other):
        if isinstance(cfg_other, dict):
            ...
            now_cfg = [
                self.dfs(d) for d in cfg_other if (not inspect.ismodule(d))
            ]
        else:
